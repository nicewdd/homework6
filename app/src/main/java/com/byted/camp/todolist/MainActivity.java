...
public class MainActivity extends AppCompatActivity {
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        try{
            cursor =db.query(TodoContract.FeedEntry.TABLE_NAME,
                    new String[]{TodoContract.FeedEntry._ID,TodoContract.FeedEntry.COLUMN_CONTENT,TodoContract.FeedEntry.COLUMN_STATE,TodoContract.FeedEntry.COLUMN_DATE},
                    null,null,
                    null,null,
            ...
            while(cursor.moveToNext()){
                ...
                int intState = cursor.getInt(cursor.getColumnIndex(TodoContract.FeedEntry.COLUMN_STATE));
                long id = cursor.getLong(cursor.getColumnIndex(TodoContract.FeedEntry._ID));

                Note note = new Note(id);
                note.setContent(content);
                note.setDate(new Date(dateMs*1000));
    ...
    private void deleteNote(Note note) {
        ...
        int deleteRow = db.delete(TodoContract.FeedEntry.TABLE_NAME,selection,selectionArgs);
        notesAdapter.refresh(loadNotesFromDatabase());
        db.close();;
    }
